...
# Strategy: Select file with highest version number in path OR filename (v4.2 > v4.1 > v3.0)
...
while IFS= read -r file; do
    ...
    if echo "$filename" | grep -E "$SPEC_REGEX" > /dev/null; then
        ...
        # IMPROVED: Extract version number from BOTH path and filename with support for semantic versioning
        # Supports: /v4/, /v4.2/, /v4.2.1/ in paths
        # Also supports: swagger-v2.1.json, spec-v3.0.yaml in filenames
        version_num=0
        version_string=""

        # First try to extract from path (e.g., /v4.2/ or /v4/)
        if [[ "$file" =~ /v([0-9]+(\.[0-9]+)*)/ ]]; then
            version_string="${BASH_REMATCH[1]}"
            echo "[DEBUG]   Found version in path: v$version_string" >&2
        # If not found in path, try to extract from filename (e.g., swagger-v2.1.json)
        elif [[ "$filename" =~ -v([0-9]+(\.[0-9]+)*)\.(json|yaml|yml)$ ]] || [[ "$filename" =~ _v([0-9]+(\.[0-9]+)*)\.(json|yaml|yml)$ ]]; then
            version_string="${BASH_REMATCH[1]}"
            echo "[DEBUG]   Found version in filename: v$version_string" >&2
        fi
        ...
        if [ "$version_num" -gt "$BEST_VERSION_NUM" ]; then
            is_better=1
            echo "[DEBUG]   -> Better version ($version_num > $BEST_VERSION_NUM)" >&2
        elif [ "$version_num" -eq "$BEST_VERSION_NUM" ] && [ "$commit_timestamp" -gt "$BEST_COMMIT_TIMESTAMP" ]; then
            is_better=1
            echo "[DEBUG]   -> Same version but newer commit ($commit_timestamp > $BEST_COMMIT_TIMESTAMP)" >&2
        fi
